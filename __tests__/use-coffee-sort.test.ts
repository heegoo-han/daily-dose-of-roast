...
describe("useCoffeeSort", () => {
  it("기본 정렬이 평점 순이고 첫 번째 카페 평점이 4.6이다", () => {
    const { result } = renderHook(() => useCoffeeSort(CAFES))
    expect(result.current.sort).toBe("rating")
    expect(result.current.sortedCafes[0].rating).toBe(4.6)
    expect(result.current.sortedCafes[1].rating).toBe(4.2)
  })

  it("거리 순으로 변경하면 가장 가까운 카페가 첫 번째가 된다", () => {
    const { result } = renderHook(() => useCoffeeSort(CAFES))
    act(() => result.current.setSort("distance"))
    expect(result.current.sortedCafes[0].distanceM).toBe(350)
  ...
  it("인기 순으로 변경하면 리뷰 수 많은 카페가 첫 번째가 된다", () => {
    ...
    expect(result.current.sortedCafes[0].reviewCount).toBe(847)
    expect(result.current.sortedCafes[4].reviewCount).toBe(43)
  })
